...
export default function CollectionsIndex() {
  return (
    <Container className="py-14">
      ...
      <p className="mt-4 max-w-2xl text-lg text-ink/75">Ten curated bodies of work, from abstract seascapes to large-scale statement paintings.</p>
      <div className="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {collections.map(c => (
          <Link key={c.slug} href={`/collections/${c.slug}`} className="rounded-md border border-sand p-6 hover:border-ink">
            <h2 className="font-serif text-2xl text-ink">{c.name}</h2>
            <p className="mt-2 text-sm text-ink/60">{c.intro.slice(0, 120)}…</p>
            <p className="mt-3 text-xs uppercase tracking-widest text-ink/45">{artworksInCollection(c.slug).length} works</p>
          </Link>
        ))}
      </div>
    </Container>
