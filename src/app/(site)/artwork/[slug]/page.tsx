...
import { artworks, getArtwork, relatedArtworks, mockupsForArtwork, collectionName, formatPrice, dims } from '@/lib/data';
...
export default function ArtworkPage({ params }: { params: { slug: string } }) {
  ...
  const crumbs = [{ name: 'Home', path: '/' }, { name: 'Portfolio', path: '/portfolio' }, { name: collectionName(a.primaryCollection), path: `/collections/${a.primaryCollection}` }, { name: a.title, path: `/artwork/${a.slug}` }];
  return (
    <Container className="py-14">
      ...
      <div className="mt-6 grid gap-12 lg:grid-cols-2">
        <div>
          <PlaceholderImage src={a.image} alt={a.alt} ratio={a.orientation === 'portrait' ? '3 / 4' : a.orientation === 'square' ? '1 / 1' : '4 / 3'} priority />
          {mocks.length > 0 && (
            <div className="mt-6 grid grid-cols-3 gap-3">
              {mocks.map(m => <Link key={m.slug} href={`/mockups/${m.slug}`}><PlaceholderImage src={m.image} alt={m.alt} ratio="4 / 3" /></Link>)}
            </div>
          )}
        </div>
        <div>
          ...
          <p className="mt-3 text-xl text-ink/70">{a.status === 'sold' ? 'Sold' : formatPrice(a.price, a.currency)}</p>
          <dl className="mt-6 grid grid-cols-2 gap-y-3 text-sm">
            <dt className="text-ink/50">Year</dt><dd>{a.year}</dd>
            <dt className="text-ink/50">Medium</dt><dd>{a.medium}</dd>
            ...
            <dt className="text-ink/50">Status</dt><dd className="capitalize">{a.status}</dd>
          ...
          <p className="mt-4 text-sm">Part of:{' '}
            {a.collections.map((c, i) => <span key={c}>{i > 0 && ', '}<Link href={`/collections/${c}`} className="underline">{collectionName(c)}</Link></span>)}
          </p>
        </div>
      </div>
      {related.length > 0 && (
