...
export default function EmailCapture({ dark }: { dark?: boolean }) {
  ...
  if (done) return <p className={dark ? 'text-sm text-bone/80' : 'text-sm text-ink/70'}>Thank you — you&rsquo;re on the list.</p>;
  return (
    <form onSubmit={submit} className="flex gap-2">
      ...
      <input id="ec" type="email" required value={email} onChange={e => setEmail(e.target.value)} placeholder="you@email.com"
        className={`w-full rounded-full border px-4 py-2 text-sm ${dark ? 'border-bone/30 bg-transparent text-bone placeholder:text-bone/50' : 'border-ink/25 bg-white text-ink'}`} />
